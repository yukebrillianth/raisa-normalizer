...
export function PipelineTimeline({
  ...
  const phaseLabel = useMemo(() => {
    switch (phase) {
      case "idle": return "Menunggu input";
      case "uploading": return "Mengunggah audio";
      case "streaming": return "Pipeline aktif";
  ...
  const phaseColor = useMemo(() => {
    switch (phase) {
      case "streaming": return "text-its-blue";
...
function CandidatesSection({ candidates }: { candidates: RetrievalCandidate[] }) {
  ...
  return (
    <div className="px-4 py-3 border-t border-surface-3">
      ...
      {expanded && (
        <div className="mt-2 space-y-2 animate-fade-in" data-testid="retrieval-candidates">
          {candidates.map((c) => (
            <div key={c.rank} className="rounded bg-surface-1 border border-surface-3 p-2.5 text-xs">
              ...
              <p className="text-text-secondary leading-relaxed">
                <span className="font-medium text-text-primary">Q:</span> {c.question}
              </p>
              <p className="text-text-muted leading-relaxed mt-0.5">
                <span className="font-medium text-text-secondary">A:</span> {c.answer}
              </p>
            </div>
          ))}
        </div>
